...
//        }
    ...
    public static String outputFormat(String date){
        if(TextUtils.isEmpty(date))
            return "";
        String month=date.split("-")[1].length()<2?"0"+date.split("-")[1] : date.split("-")[1];
        String day=date.split("-")[2].length()<2?"0"+date.split("-")[2] : date.split("-")[2];
        String week = getWeekByFormat(date);
     ...
     * @return
     */
    public static String getNextMonthFirstOfMouth(Date date) {
        String str = "";
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
