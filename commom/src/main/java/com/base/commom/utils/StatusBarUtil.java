...
public class StatusBarUtil {
    ...
    public static void darkMode(Activity activity, boolean dark, float alpha) {
        if (isFlyme4Later()) {
            ...
            setTranslucentView((ViewGroup) activity.getWindow().getDecorView(), 0, alpha);
        } else {
            immersive(activity.getWindow(), 0, alpha);
        }
    }
    ...
    public static void darkMode(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (isFlyme4Later()) {
            ...
            window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            setTranslucentView((ViewGroup) window.getDecorView(), color, 0.5f);
        }
//        if (Build.VERSION.SDK_INT >= 21) {
//            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
//            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
...
//        window.setStatusBarColor(Color.TRANSPARENT);
    ...
    public static void setPaddingAndVaule(Context context, View view, int dp) {
        if (Build.VERSION.SDK_INT >= MIN_API) {
            view.setPadding(view.getPaddingLeft(), view.getPaddingTop() + getStatusBarHeight(context) + dp,
                    view.getPaddingRight(), view.getPaddingBottom());
        }
    }

    /** 增加View的paddingTop,增加的值为状态栏高度 */
    public static void setPaddingAndValueBottom(Context context, View view, int dp) {
        if (Build.VERSION.SDK_INT >= MIN_API) {
            view.setPadding(view.getPaddingLeft(), view.getPaddingTop() + getStatusBarHeight(context) + dp,
                    view.getPaddingRight(), view.getPaddingBottom()+ dp);
        }
    }
    ...
    /** 增加View的高度以及paddingTop,增加的值为状态栏高度.一般是在沉浸式全屏给ToolBar用的 */
    public static void setHeightWithValues(Context context, View view, int dp) {
        if (Build.VERSION.SDK_INT >= MIN_API) {
            ViewGroup.LayoutParams lp = view.getLayoutParams();
            lp.height = dp;//增高
        }
    ...
    public static void setHeight(Context context, View view, int dp) {
        if (Build.VERSION.SDK_INT >= MIN_API) {
            ViewGroup.LayoutParams lp = view.getLayoutParams();
            lp.height = lp.height + getStatusBarHeight(context) + dp;//增高
        }
    }
    /** 增加View的高度以及paddingTop,增加的值为状态栏高度.一般是在沉浸式全屏给ToolBar用的 */
    ...
    /** 增加View的paddingTop,增加的值为状态栏高度 */
    public static void setMarginAndVaule(Context context, View view, int dp) {
        if (Build.VERSION.SDK_INT >= MIN_API) {
            ViewGroup.LayoutParams lp = view.getLayoutParams();
            if (lp instanceof ViewGroup.MarginLayoutParams) {
                ((ViewGroup.MarginLayoutParams) lp).topMargin += getStatusBarHeight(context) + dp;//增高
            }
            view.setLayoutParams(lp);
        }
    }
